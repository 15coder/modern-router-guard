import type {
  RouterClient,
  ConnectedDevice,
  WifiSettings,
  NetworkStats,
  RouterInfo,
} from "./types";
import { guessDeviceType, guessVendor } from "./utils";

function md5(str: string): string {
  // Simple MD5 for TP-Link auth — production apps should use a proper library
  // This is a lightweight implementation for router password hashing
  function safeAdd(x: number, y: number) {
    const lsw = (x & 0xffff) + (y & 0xffff);
    const msw = (x >> 16) + (y >> 16) + (lsw >> 16);
    return (msw << 16) | (lsw & 0xffff);
  }
  function bitRotateLeft(num: number, cnt: number) {
    return (num << cnt) | (num >>> (32 - cnt));
  }
  function md5cmn(q: number, a: number, b: number, x: number, s: number, t: number) {
    return safeAdd(bitRotateLeft(safeAdd(safeAdd(a, q), safeAdd(x, t)), s), b);
  }
  function md5ff(a: number, b: number, c: number, d: number, x: number, s: number, t: number) {
    return md5cmn((b & c) | (~b & d), a, b, x, s, t);
  }
  function md5gg(a: number, b: number, c: number, d: number, x: number, s: number, t: number) {
    return md5cmn((b & d) | (c & ~d), a, b, x, s, t);
  }
  function md5hh(a: number, b: number, c: number, d: number, x: number, s: number, t: number) {
    return md5cmn(b ^ c ^ d, a, b, x, s, t);
  }
  function md5ii(a: number, b: number, c: number, d: number, x: number, s: number, t: number) {
    return md5cmn(c ^ (b | ~d), a, b, x, s, t);
  }
  function computeMD5(m: number[], l: number) {
    m[l >> 5] |= 0x80 << (l % 32);
    m[(((l + 64) >>> 9) << 4) + 14] = l;
    let a = 1732584193, b = -271733879, c = -1732584194, d = 271733878;
    for (let i = 0; i < m.length; i += 16) {
      const aa = a, bb = b, cc = c, dd = d;
      a = md5ff(a, b, c, d, m[i], 7, -680876936);
      d = md5ff(d, a, b, c, m[i + 1], 12, -389564586);
      c = md5ff(c, d, a, b, m[i + 2], 17, 606105819);
      b = md5ff(b, c, d, a, m[i + 3], 22, -1044525330);
      a = md5ff(a, b, c, d, m[i + 4], 7, -176418897);
      d = md5ff(d, a, b, c, m[i + 5], 12, 1200080426);
      c = md5ff(c, d, a, b, m[i + 6], 17, -1473231341);
      b = md5ff(b, c, d, a, m[i + 7], 22, -45705983);
      a = md5ff(a, b, c, d, m[i + 8], 7, 1770035416);
      d = md5ff(d, a, b, c, m[i + 9], 12, -1958414417);
      c = md5ff(c, d, a, b, m[i + 10], 17, -42063);
      b = md5ff(b, c, d, a, m[i + 11], 22, -1990404162);
      a = md5ff(a, b, c, d, m[i + 12], 7, 1804603682);
      d = md5ff(d, a, b, c, m[i + 13], 12, -40341101);
      c = md5ff(c, d, a, b, m[i + 14], 17, -1502002290);
      b = md5ff(b, c, d, a, m[i + 15], 22, 1236535329);
      a = md5gg(a, b, c, d, m[i + 1], 5, -165796510);
      d = md5gg(d, a, b, c, m[i + 6], 9, -1069501632);
      c = md5gg(c, d, a, b, m[i + 11], 14, 643717713);
      b = md5gg(b, c, d, a, m[i], 20, -373897302);
      a = md5gg(a, b, c, d, m[i + 5], 5, -701558691);
      d = md5gg(d, a, b, c, m[i + 10], 9, 38016083);
      c = md5gg(c, d, a, b, m[i + 15], 14, -660478335);
      b = md5gg(b, c, d, a, m[i + 4], 20, -405537848);
      a = md5gg(a, b, c, d, m[i + 9], 5, 568446438);
      d = md5gg(d, a, b, c, m[i + 14], 9, -1019803690);
      c = md5gg(c, d, a, b, m[i + 3], 14, -187363961);
      b = md5gg(b, c, d, a, m[i + 8], 20, 1163531501);
      a = md5gg(a, b, c, d, m[i + 13], 5, -1444681467);
      d = md5gg(d, a, b, c, m[i + 2], 9, -51403784);
      c = md5gg(c, d, a, b, m[i + 7], 14, 1735328473);
      b = md5gg(b, c, d, a, m[i + 12], 20, -1926607734);
      a = md5hh(a, b, c, d, m[i + 5], 4, -378558);
      d = md5hh(d, a, b, c, m[i + 8], 11, -2022574463);
      c = md5hh(c, d, a, b, m[i + 11], 16, 1839030562);
      b = md5hh(b, c, d, a, m[i + 14], 23, -35309556);
      a = md5hh(a, b, c, d, m[i + 1], 4, -1530992060);
      d = md5hh(d, a, b, c, m[i + 4], 11, 1272893353);
      c = md5hh(c, d, a, b, m[i + 7], 16, -155497632);
      b = md5hh(b, c, d, a, m[i + 10], 23, -1094730640);
      a = md5hh(a, b, c, d, m[i + 13], 4, 681279174);
      d = md5hh(d, a, b, c, m[i], 11, -358537222);
      c = md5hh(c, d, a, b, m[i + 3], 16, -722521979);
      b = md5hh(b, c, d, a, m[i + 6], 23, 76029189);
      a = md5hh(a, b, c, d, m[i + 9], 4, -640364487);
      d = md5hh(d, a, b, c, m[i + 12], 11, -421815835);
      c = md5hh(c, d, a, b, m[i + 15], 16, 530742520);
      b = md5hh(b, c, d, a, m[i + 2], 23, -995338651);
      a = md5ii(a, b, c, d, m[i], 6, -198630844);
      d = md5ii(d, a, b, c, m[i + 7], 10, 1126891415);
      c = md5ii(c, d, a, b, m[i + 14], 15, -1416354905);
      b = md5ii(b, c, d, a, m[i + 5], 21, -57434055);
      a = md5ii(a, b, c, d, m[i + 12], 6, 1700485571);
      d = md5ii(d, a, b, c, m[i + 3], 10, -1894986606);
      c = md5ii(c, d, a, b, m[i + 10], 15, -1051523);
      b = md5ii(b, c, d, a, m[i + 1], 21, -2054922799);
      a = md5ii(a, b, c, d, m[i + 8], 6, 1873313359);
      d = md5ii(d, a, b, c, m[i + 15], 10, -30611744);
      c = md5ii(c, d, a, b, m[i + 6], 15, -1560198380);
      b = md5ii(b, c, d, a, m[i + 13], 21, 1309151649);
      a = md5ii(a, b, c, d, m[i + 4], 6, -145523070);
      d = md5ii(d, a, b, c, m[i + 11], 10, -1120210379);
      c = md5ii(c, d, a, b, m[i + 2], 15, 718787259);
      b = md5ii(b, c, d, a, m[i + 9], 21, -343485551);
      a = safeAdd(a, aa); b = safeAdd(b, bb);
      c = safeAdd(c, cc); d = safeAdd(d, dd);
    }
    return [a, b, c, d];
  }
  function str2binl(str: string) {
    const bin: number[] = [];
    const mask = (1 << 8) - 1;
    for (let i = 0; i < str.length * 8; i += 8)
      bin[i >> 5] |= (str.charCodeAt(i / 8) & mask) << (i % 32);
    return bin;
  }
  function binl2hex(binarray: number[]) {
    const hexTab = "0123456789abcdef";
    let str = "";
    for (let i = 0; i < binarray.length * 4; i++) {
      str += hexTab.charAt((binarray[i >> 2] >> ((i % 4) * 8 + 4)) & 0xf) +
        hexTab.charAt((binarray[i >> 2] >> ((i % 4) * 8)) & 0xf);
    }
    return str;
  }
  return binl2hex(computeMD5(str2binl(str), str.length * 8));
}

export class TpLinkClient implements RouterClient {
  private ip: string;
  private username: string;
  private password: string;
  private stok: string = "";
  private baseUrl: string;

  constructor(ip: string, username: string, password: string) {
    this.ip = ip;
    this.username = username;
    this.password = password;
    this.baseUrl = `http://${ip}`;
  }

  private async request(endpoint: string, body: object, withStok = false): Promise<any> {
    const url = withStok
      ? `${this.baseUrl}/cgi-bin/luci/;stok=${this.stok}${endpoint}`
      : `${this.baseUrl}${endpoint}`;

    const response = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    });

    if (!response.ok) throw new Error(`TP-Link HTTP error: ${response.status}`);
    const json = await response.json();
    if (json.error_code !== 0) {
      throw new Error(`TP-Link API error: ${json.error_code}`);
    }
    return json;
  }

  async login(): Promise<void> {
    const result = await this.request("/cgi-bin/luci/;stok=/login", {
      method: "do",
      login: { username: this.username, password: md5(this.password) },
    });
    this.stok = result.stok;
  }

  async getDevices(): Promise<ConnectedDevice[]> {
    const result = await this.request("/admin/wireless", {
      method: "get",
      hosts_info: { table: "host_info" },
    }, true);

    const hosts: any[] = result.hosts_info?.host_info ?? [];
    return hosts.map((h: any, i: number) => ({
      id: h.mac ?? `dev-${i}`,
      name: h.hostname || h.name || "جهاز غير معروف",
      mac: h.mac ?? "",
      ip: h.ip ?? "",
      deviceType: guessDeviceType(h.hostname ?? h.name ?? ""),
      vendor: guessVendor(h.mac ?? ""),
      blocked: h.blocked === "1",
      connected: true,
      isNew: false,
    }));
  }

  async getWifiSettings(): Promise<WifiSettings> {
    const result = await this.request("/admin/wireless", {
      method: "get",
      wireless: { name: ["wlan", "wlan_access_point"] },
    }, true);

    const wlan = result.wireless ?? {};
    return {
      ssid_2g: wlan["2g"]?.ssid ?? "MyNetwork_2G",
      ssid_5g: wlan["5g"]?.ssid ?? "MyNetwork_5G",
      password_2g: wlan["2g"]?.password ?? "",
      password_5g: wlan["5g"]?.password ?? "",
      hidden_2g: wlan["2g"]?.hidden === "1",
      hidden_5g: wlan["5g"]?.hidden === "1",
      band: "both",
      channel_2g: parseInt(wlan["2g"]?.channel ?? "0"),
      channel_5g: parseInt(wlan["5g"]?.channel ?? "0"),
      security: "WPA2",
    };
  }

  async updateWifiSettings(settings: Partial<WifiSettings>): Promise<void> {
    await this.request("/admin/wireless", {
      method: "set",
      wireless: {
        "2g": {
          ssid: settings.ssid_2g,
          password: settings.password_2g,
          hidden: settings.hidden_2g ? "1" : "0",
        },
        "5g": {
          ssid: settings.ssid_5g,
          password: settings.password_5g,
          hidden: settings.hidden_5g ? "1" : "0",
        },
      },
    }, true);
  }

  async getNetworkStats(): Promise<NetworkStats> {
    const result = await this.request("/admin/status", {
      method: "get",
      network: { name: ["wan_status", "lan_info"] },
    }, true);

    const wan = result.network?.wan_status ?? {};
    return {
      downloadMbps: parseFloat(wan.rx_speed ?? "0") / 1024 / 1024,
      uploadMbps: parseFloat(wan.tx_speed ?? "0") / 1024 / 1024,
      connectedDevices: parseInt(wan.online_client ?? "0"),
      uptimeSeconds: parseInt(wan.online_time ?? "0"),
      wanIp: wan.ip_addr ?? "",
      dns1: wan.dns1 ?? "8.8.8.8",
      dns2: wan.dns2 ?? "8.8.4.4",
    };
  }

  async blockDevice(mac: string): Promise<void> {
    await this.request("/admin/wireless", {
      method: "set",
      host_filter: { type: "black", host: [{ mac }] },
    }, true);
  }

  async unblockDevice(mac: string): Promise<void> {
    await this.request("/admin/wireless", {
      method: "del",
      host_filter: { host: [{ mac }] },
    }, true);
  }

  async restartRouter(): Promise<void> {
    await this.request("/admin/system", {
      method: "do",
      system: { reboot: null },
    }, true);
  }

  async getRouterInfo(): Promise<RouterInfo> {
    const result = await this.request("/admin/firmware", {
      method: "get",
      firmware_version: null,
    }, true);

    return {
      model: result.firmware_version?.model ?? "TP-Link",
      firmware: result.firmware_version?.firmware_version ?? "Unknown",
      type: "tp-link",
    };
  }
}
